...
[Blake Shaffer made a nice vaccine visualization](https://twitter.com/bcshaffer/status/1386799473972572163) for age-specific vaccination rates that we like, so we are shamelessly reproducing it here to auto-update.
...
dose_levels <- c("2 doses","1st dose","Unvaccinated")

plot_data <- vaccine_age %>%
  filter(week_end==max(week_end)) %>%
  filter(!(age %in% c("Unknown", "Not reported"))) %>%
  group_by(pruid,prename,age) %>%
  summarize(`1st dose`=sum(numtotal_1dose,na.rm=TRUE),
            `2 doses`=sum(numtotal_2doses,na.rm=TRUE)) %>%
  left_join(pop,by=c("pruid"="GeoUID","age"="age")) %>%
  pivot_longer(c("1st dose","2 doses")) %>%
...
hi <- tibble(name=c("Wild type","VOC"),R_0=c(2.6,3.9)) %>%
  mutate(naive=1-1/(R_0*0.75)) %>%
  mutate(min=naive/0.8,max=naive/0.95)

library(ggpattern)
#ggplot(pd %>% filter(GEO=="British Columbia")) +
ggplot(pd %>% filter(GEO %in% c("British Columbia"))) +
  theme_bw() +
  ...
  geom_rect_pattern(data=hi,aes(xmin=0,xmax=1,ymin=min,ymax=max,pattern=name),fill=NA,color="black",
                    pattern_density=0.1,pattern_spacing=0.01) +
  scale_pattern_manual(values=c("VOC"="crosshatch","Wild type"="circle"))+
