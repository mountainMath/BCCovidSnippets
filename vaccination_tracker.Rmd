...
```{r}
age_levels <- c("fill1","fill2","fill3","0-17", "18-29", "30-39", "40-49", "50-59", "60-69", "70-79", "80+")
...
plot_data <- vaccine_age %>%
  ...
  filter(!(age %in% c("Unknown", "Not reported","All Ages"))) %>%
...
uk_mean <- world_vaccine %>% 
  filter(iso_code=="GBR",
         !is.na(people_vaccinated_per_hundred)) %>% 
  filter(date==max(date)) %>%
  mutate(r=people_vaccinated_per_hundred/100,
         rf=people_fully_vaccinated_per_hundred/100) 
is_mean <- world_vaccine %>% 
  filter(iso_code=="ISR",
         !is.na(people_vaccinated_per_hundred)) %>% 
  filter(date==max(date)) %>%
  mutate(r=people_vaccinated_per_hundred/100,
...
vaccine_comparison <- tibble(Region=c("British Columbia","United Kingdom","Israel"),
                             `Fully vaccintated`=c(bc_current_f,uk_mean$rf,is_mean$rf),
                             `Partially vaccinated`=c(bc_current-bc_current_f,
                                                      uk_mean$r-uk_mean$rf,
                                                      is_mean$r-is_mean$rf)) %>%
  mutate(Unvaccinated=1-`Fully vaccintated`-`Partially vaccinated`) %>%
  pivot_longer(-Region) %>%
  mutate(Region=factor(Region,levels=c("British Columbia","United Kingdom","Israel"))) %>%
  mutate(name-factor(name,levels=c("Fully vaccintated","Partially vaccinated","Unvaccinated")))
...
label_data <- tibble(y=c(mean_vaccine_level_d$share,bc_current,uk_mean$r,is_mean$r),
                     labels=c(paste0("BC average\n as of ",max(vaccine_age$week_end)),
                              paste0("BC average\n as of ",bc_current_date),
                              "UK average",
                              "Israel average"))
...
ggplot() +
  ...
  patchwork::inset_element(g, 0.05, 0.6, 0.4, 0.95)
```
