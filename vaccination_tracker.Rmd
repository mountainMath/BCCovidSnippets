...
pop_data <- get_cansim("17-10-0005") %>%
  ...
  select(Province=GEO,GEO.abb,Population=VALUE)
...
```{r vaccination-dose}
vaccination_data <- get_canada_covid_working_group_timeseries("avaccine") %>%
  left_join(pop_data,by="Province") %>%
  mutate(Date=date_vaccine_administered) %>%
  mutate(Share=cumulative_avaccine/Population*100)

vaccination_doses <- get_canada_covid_working_group_timeseries("dvaccine") 
g <- vaccination_data %>%
  left_join(vaccination_doses %>% 
              select(Province,Date=date_vaccine_distributed,total=cumulative_dvaccine),
            by=c("Date","Province")) %>%
  mutate(Administered=Share,Warehoused=total/Population*100-Administered) %>%
...
vaccination_data <- get_canada_covid_working_group_timeseries("cvaccine") %>%
  left_join(pop_data,by="Province") %>%
  mutate(Date=date_vaccine_completed) %>%
  mutate(Share=cumulative_cvaccine/Population)
