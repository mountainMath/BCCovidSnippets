...
joint_data_for <- function(HRs, WPs,label=NULL){
  ...
  wd<-wastewater_data %>%
    ...
    summarise(Concentration=sum(Concentration*weight),
              `Flow adjusted`=sum(`Flow adjusted`*weight),
              .groups="drop")
  cs <- bc_cases %>% 
    filter(HR %in% HRs) %>%
    group_by(Date) %>%
...
wastewater_colours<- setNames(c(sanzo::trios$c157,"darkgrey"),c("Cases","Concentration","Flow adjusted","Rainfall"))
...
pd <- plot_data %>%
  ...
  mutate(name=factor(name,levels=c("Cases","Concentration","Flow adjusted","Rainfall") %>% rev)) %>%
  group_by(name,region) %>%
  filter(Date>=beginning_date) %>%
  left_join((.) %>% filter(Date>=change_methods_date,Date<=change_methods_end_date) %>%
              group_by(name,region) %>%
              summarise(mean_value=mean(value,na.rm=TRUE)),by=c("name","region")) %>%
...
pd %>% 
  ...
  facet_wrap(~region) +
  theme_bw() +
  scale_colour_manual(values=wastewater_colours[(unique(as.character(pd$name)))]) +
  scale_y_continuous() +
  theme(legend.position = "bottom",
...
pd %>% filter(Date>=as.Date("2021-12-01")) %>%
  ...
  geom_vline(xintercept = change_methods_date,linetype="dotted") +
  geom_line() +
  geom_point(shape=21,data=~filter(.,name %in% c("Concentration","Flow adjusted"))) +
  facet_wrap(~region) +
  theme_bw() +
  scale_colour_manual(values=wastewater_colours[(unique(as.character(pd$name)))]) +
  scale_y_continuous() +
