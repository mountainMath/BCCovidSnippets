...
joint_data_for <- function(HRs, WPs,label=NULL){
  ...
  cs <- bc_cases %>% 
    ...
    mutate(Trend=add_stl_trend_m(Cases+1)$trend-1)
...
weather_data<-weathercan::weather_dl(51442,interval = "day",start=min(wastewater_data$Date)-3) %>%
  mutate(Date=date) %>%
...
plot_data %>%
  ...
  geom_line(data=weather_data %>% filter(Date>=as.Date("2020-10-01")),
            colour="grey",size=0.5,line_type="dotted") +
  geom_line() +
  facet_wrap(~region) +
  theme_bw() +
...
#ccf(plot_data$Cases,plot_data$Value)
```
