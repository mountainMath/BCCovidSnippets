...
stl_tail <- get_stl_trend_uncertainty(all_data %>% arrange(Date) %>% pull(Cases),level=0.5,gr_add=c(-0.01,0.01)) %>%
...
stl_tail <- data %>%
  group_by(HA,HR) %>%
  arrange(Date) %>%
  group_map(~get_stl_trend_uncertainty(.x$Cases+1,level=0.5,gr_add=c(-0.01,0.01)) %>%
              mutate(HA=.y$HA,HR=.y$HR)) %>%
  bind_rows() %>%
  mutate_at(c("min","max"),function(d)pmax(0,d-1)) %>%
  mutate(Date=max(all_data$Date)+d) %>%
...
stl_tail <- dd %>%
  group_by(HA,AG) %>%
  arrange(Date) %>%
  group_map(~get_stl_trend_uncertainty(.x$Cases+5,level=0.5,gr_add=c(-0.01,0.01)) %>%
              mutate(HA=.y$HA,AG=.y$AG)) %>%
  bind_rows() %>%
  mutate_at(c("min","max"),function(d)pmax(0,d-5)) %>%
  mutate(Date=max(all_data$Date)+d) %>%
  filter(Date>=as.Date("2020-12-01")) %>%
  left_join(dd  %>% filter(Date==max(Date)) %>% select(HA,AG,Population),by=c("HA","AG")) %>%
  mutate_at(c("min","max"),function(d)d/.$Population*100000)
