...
bc_cases_age_date %>%
  ...
  labs(title=paste0("Case trend lines by age group in British Columbia (up to ",strftime(max(data$Date),"%a %b %d"),")"),
       colour="Age group",
...
bc_cases_prevalence_date %>%
  ...
  scale_x_date(labels=function(d)strftime(d,"%b"),breaks="month") +
  labs(title=paste0("Relative incidence by age group in British Columbia (up to ",strftime(max(data$Date),"%a %b %d"),")"),
       colour="Age group",
