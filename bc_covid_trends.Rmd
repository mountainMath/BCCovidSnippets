...
n501y <- read_csv(here::here("data/n501y.csv"))
model.n501y <- lm(log(ratio_voc)~Date,data=n501y %>% filter(as.integer(Week)>=7))
prediction.n501y <- tibble(Date=seq(as.Date("2021-02-01"),Sys.Date(),by="day")) %>%
  mutate(share_voc = predict(model.n501y,newdata = .) %>% exp %>% ratio_to_share) 
```
...
g <- data %>% 
  ...
  geom_line(data=~filter(.,type=="Wild Type",Date<=max(n501y$Date)),aes(color=type),size=1) +
  geom_line(data=~filter(.,type=="Wild Type",Date>max(n501y$Date)),aes(color=type),size=1,linetype="dotted") +
  ...
  scale_color_manual(values=c("Cases"="darkgrey","Cleaned"="darkgrey",
                     ...
                     labels=c("Cases"="Reported cases","Cleaned"="Adjused for weekly pattern",
...
bc_cases_prevalence_date %>%
  ...
  scale_x_date(labels=function(d)strftime(d,"%b"),breaks="month") +
  labs(title="Relative prevalence by age group",colour="Age group",
       x=NULL,y="Relative prevalence\n(share of cases by share of population)",
       caption="MountainMath, Data: BCCDC, BC Stats Population estimates 2019")
```
