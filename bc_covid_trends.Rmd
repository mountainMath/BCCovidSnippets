...
model.n501y <- lm(log(ratio_voc)~Date,data=n501y %>% filter(Week>=7))
prediction.n501y <- tibble(Date=seq(as.Date("2021-02-01"),Sys.Date(),by="day")) %>%
  mutate(ratio_voc =- predict(model.n501y,newdata = .) %>% exp) %>%
...
g <- data %>% 
  ...
  geom_line(data=~filter(.,type=="Wild Type"),aes(color=type),size=1) +
  theme_bw() +
  ...
  scale_color_manual(values=c("Cases"="darkgrey","Cleaned"="darkgrey",
                     ...
                     labels=c("Cases"="Cases","Cleaned"="Adjused for weekly pattern",
                              "Trend"="Trend","Wild Type"="Wild Type")) +
